...
public class ProductoRepository implements ProductoDAO{
    ...
    public void crearProductos(Producto Producto){
        String query = "INSERT INTO producto (nombre, fecha_creacion, id_tipo_producto) values (?,?,?)";
        jdbcTemplate.update(query, Producto.getNombre(),Producto.getFechaCreacion(),Producto.getTipoProducto().getId());
    }
